...
namespace koz
{
    ...
    {
        private MySqlConnection conn = new MySqlConnection("server=localhost;database=dbkoz;username=root;password=;"); //Change database name
        public AdminForm()
        {
            InitializeComponent();
        }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (confirm == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    string StudentNo = txtusername.Text;
                    string Firstname = txtfirstname.Text;
                    string Lastname = txtlastname.Text;
                    string MiddleName = txtmiddlename.Text;

                    // Ensure the correct date format MONTH DAY YEAR
                    string Birthdate = bday.Value.ToString("MM dd, yyyy");
                  ...
                  WHERE username = @StudentNo";
                    ...
                    txtusername.Clear();
                    textBox2.Clear();
                    comboBox2.SelectedIndex = -1;  // Clear the course selection
                    comboBox1.SelectedIndex = -1; // Clear the section selection
                    bday.Value = DateTime.Today; // Reset the birthdate picker
